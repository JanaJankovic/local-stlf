...
def prepare_data(args):
    ...
    test_dataset = TensorDataset(*X_test_tensors, torch.zeros(n_test, args['horizon'], 1).to(DEVICE))

    train_loader = DataLoader(train_dataset, batch_size=args['batch_size'], shuffle=False)
    val_loader = DataLoader(val_dataset, batch_size=args['batch_size'], shuffle=False)
    test_loader = DataLoader(test_dataset, batch_size=args['batch_size'], shuffle=False)

    return train_loader, val_loader, test_loader, (y_train, y_val, y_test), scalers_X, scalers_y, X_train_tensors, X_val_tensors, X_test_tensors
