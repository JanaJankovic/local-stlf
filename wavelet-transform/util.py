...
def evaluate_model(
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            for b in range(B):
                ...
                recon = swt_reconstruct_from_preds(
                    band_preds, wavelet=wavelet, scaler_y=scaler_y
                )
                preds_scaled.append(
                    torch.tensor(recon[-s:], dtype=torch.float32)
                )  # [s]
    ...
    return y_pred, calculate_metrics(
        y_true, y_pred, elapsed_time=elapsed, type=data_type
    )
    ...
    with torch.no_grad():
        for t in range(0, len(y_test) - forecast_steps + 1, forecast_steps):
            ...
            for j, (a, d) in enumerate(coeffs):
                ...
                band_tensor = (
                    torch.tensor(band, dtype=torch.float32).unsqueeze(0).unsqueeze(-1)
                )
                input_bands.append(band_tensor)

            x_tensor = torch.stack(input_bands, dim=1).to(DEVICE)  # [1, bands, W, 1]
            out = (
                model(x_tensor).squeeze(0).squeeze(-1).cpu().numpy()
            )  # [bands, forecast_steps]
            ...
            true_recon_coeffs = [
                (true_coeffs[i][0], true_coeffs[i][1]) for i in range(len(pred_coeffs))
            ]
