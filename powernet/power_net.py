...
class PowerNet(nn.Module):
    def __init__(self, input_size_meta, input_size_lstm=1, lstm_hidden=128, lstm_layers=2,
    ...
    def forward(self, seq_x, meta_x):
        ...
        seq_encoded = lstm_out[:, -1, :]  # Last time step output

        # MLP encoding
        meta_encoded = self.mlp(meta_x)

        # Fusion and final prediction
        combined = torch.cat((seq_encoded, meta_encoded), dim=1)
